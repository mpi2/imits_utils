package IMITS::Util::CigarParser;
## no critic(RequireUseStrict,RequireUseWarnings)
{
    $IMITS::Util::CigarParser::VERSION = '0.044';
}
## use critic
...
sub _build_query_primer_rx {
    ...
    if ( @primer_names == 0 ) {
        if ( $self->strict_mode ) {
            IMITS::Exception->throw( 'primers must be set in strict mode' );
        }
        else {
...
sub parse_query_id {
    ...
    if ( $self->strict_mode and ( not defined $well or not defined $primer ) ) {
        IMITS::Exception->throw( "Failed to parse query_id: $query_id" );
    }
...
sub parse_cigar {
    ...
    @parsed{ @CIGAR_FIELDS } = $cigar_str =~ $CIGAR_RX
        or IMITS::Exception->throw( "failed to parse CIGAR '$cigar_str'" );
...
sub input_fh {
    ...
    if ( ! ref $input ) {
        ...
        IMITS::Exception->throw( "Don't know how to read from a " . ref $input );
    }
