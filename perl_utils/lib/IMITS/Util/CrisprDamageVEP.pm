package IMITS::Util::CrisprDamageVEP;
## no critic(RequireUseStrict,RequireUseWarnings)
{
    $Util::CrisprDamageVEP::VERSION = '0.044';
}
## use critic
...
use Moose;
use IMITS::Util::DrawPileupAlignment;
use IMITS::Util::MergeVariantsVCF;
use Bio::SeqIO;
use MooseX::Types::Path::Class::MoreCoercions qw/AbsDir/;
use IPC::Run 'run';
use List::MoreUtils qw( any );
use IMITS::Util::Constants qw(
    $BWA_MEM_CMD
    $SAMTOOLS_CMD
    $BCFTOOLS_CMD
    $VEP_CMD
...
has pileup_parser => (
    is  => 'rw',
    isa => 'IMITS::Util::DrawPileupAlignment',
);
...
has merge_vcf_util => (
    is  => 'rw',
    isa => 'IMITS::Util::MergeVariantsVCF',
);
...
sub parse_pileup_file {
    ...
    my $pileup_parser = IMITS::Util::DrawPileupAlignment->new(
...
sub merge_variants {
    ...
    $work_dir->mkpath;
    my $merge_vcf_util = IMITS::Util::MergeVariantsVCF->new(
...
sub variant_effect_predictor {
    ...
    my @vep_command = (
        ...
        '--plugin', 'IMITS::Util::VEPPlugin::MutantProteinSeqs,' . $self->dir->stringify . '/',
